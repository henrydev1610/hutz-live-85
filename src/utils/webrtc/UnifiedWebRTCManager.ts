...
export class UnifiedWebRTCManager {
  ...
  async connectToHost(): Promise<void> {
    ...
    try {
      const hostId = 'host';
      console.log(`🎯 [PART] Starting handshake with: ${hostId}`);import { unifiedWebSocketService } from '@/services/UnifiedWebSocketService';
import { ConnectionHandler } from './ConnectionHandler';
import { SignalingHandler } from './SignalingHandler';
import { ParticipantManager } from './ParticipantManager';
import { WebRTCCallbacks } from './WebRTCCallbacks';
...
export class UnifiedWebRTCManager {
  ...
  async initializeAsParticipant(sessionId: string, participantId: string, stream?: MediaStream): Promise<void> {
    ...
    try {
      ...
      // CRIA O RTCPeerConnection CEDO (patch #1)
      const pc = this.connectionHandler.createPeerConnection('host'); 
      // aqui "host" é o peer alvo — podemos ajustar se necessário

      // ADICIONA TRACKS ANTES DE CRIAR OFFER (patch #2)
      this.localStream.getTracks().forEach(track => {
        pc.addTrack(track, this.localStream!);
      });
      console.log(`📹 [PART] Tracks adicionados antes da offer:`, {
      ...
      this.webrtcReady = true;
      this.updateConnectionState('websocket', 'connected');
      console.log(`✅ [PART] Entrou na sala: ${sessionId}`);

      // Agora só falta chamar connectToHost() para iniciar o handshake
    } catch (error) {
      console.error(`❌ [PART] Falha na inicialização do participante:`, error);
      this.updateConnectionState('websocket', 'failed');
      this.cleanup();
      throw error;
    }
  ...
  async connectToHost(): Promise<void> {
    console.log(`🔗 [PART] Connecting to host`);
    if (!this.localStream) throw new Error('No local stream available for host connection');

    try {
      const hostId = 'host';
      await this.connectionHandler.initiateHandshake(hostId);

      // PATCH #3: Flush ICE candidates depois do setRemoteDescription
      const pc = this.peerConnections.get(hostId);
      if (pc) {
        await this.flushIceCandidates(hostId, pc);
      }
  ...
  // ... resto do arquivo igual, sem cortes (métodos de callback, reset, cleanup, validateMediaFlow, etc) ...
}
