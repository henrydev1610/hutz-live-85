...
export const LiveSessionProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Set to track which participants are visible
  ...
  const toggleParticipantVisibility = (id: string) => {
    ...
    if (isLive && broadcastWindow && !broadcastWindow.closed) {
      ...
      console.log(`[LiveSessionContext] Sending visibility change: ${id} to ${!isCurrentlyVisible}`);
  ...
  const refreshParticipants = () => {
    ...
    console.log('[LiveSessionContext] Visible participants:', Array.from(visibleParticipants));
    
    toast({
      description: "Atualizando lista de participantes...",
    });
    
    // Update the broadcast window with the current state
    if (isLive && broadcastWindow && !broadcastWindow.closed) {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === 'PARTICIPANT_JOINED') {
        ...
        if (incomingSessionId === sessionId && hasStream) {
          ...
          // Create a simple MediaStream placeholder to mark that the participant has a stream
          // The actual stream will come via WebRTC
          let dummyStream = new MediaStream();
          
          setParticipants(prev => 
            prev.map(p => p.id === participantId ? { ...p, stream: p.stream || dummyStream } : p)
          );
        ...
        // Send current data to the broadcast window
        if (broadcastWindow && !broadcastWindow.closed) {
          const visibleSelectedParticipants = selectedParticipants.map(p => ({
            ...p,
            isVisible: visibleParticipants.has(p.id)
      ...
      } else if (event.data.type === 'REQUEST_BROADCAST_DATA') {
        console.log('[LiveSessionContext] Broadcast window requested data');
        // Send current data to the broadcast window
        if (broadcastWindow && !broadcastWindow.closed) {
          const visibleSelectedParticipants = selectedParticipants.map(p => ({
            ...p,
  ...
  }, [sessionId, maxParticipants, isLive, broadcastWindow, selectedParticipants, visibleParticipants, 
      layout, backgroundColor, backgroundImage, qrCodeVisible, qrCodeImage, qrCodePosition, 
      qrCodeSize, qrCodeText, qrCodeTextPosition, qrCodeFont, qrCodeColor]);
